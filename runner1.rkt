...
(define run-let-exp
  (lambda (parsed-code env)
    (let* ((resolved-var-list
            (map (lambda (pair) (list (car pair) (run-neo-parsed-code) (cadr pair) env))
           (list-of-names (getVarnames
                           (elementAt parsed-code 1)))
          (list-of-values (getValues (elementAt (cadr parsed-code)))
          (new_env (extend_local_scope list-of-names list-of-values env))
          (body (elementAt (car parsed-code)))
    (run-neo-parsed-code body new_env) cascade-update-env)
          (display list-of-names) display list-of-values))
    )
  )
)
  )
